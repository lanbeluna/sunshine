...
import { ArrowLeft, Loader2, LockKeyhole, Mail, Sparkles } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const { signUp, isAuthenticated, isConfigured, loading, error, clearError } = useAuth();
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (result.ok) {
      if (result.session) {
        ...
        setSuccessMessage(result.message ?? '注册成功，请检查邮箱并完成确认。');
      }
    }
  };
    ...
    <main className="ql-soft-surface flex min-h-dvh justify-center px-4 py-[max(1rem,env(safe-area-inset-top))] text-[var(--ql-ink)]">
      <div className="flex w-full max-w-[430px] flex-col">
        ...
        <section className="mb-6">
          <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-3xl bg-gradient-to-br from-[var(--ql-accent)] to-[var(--ql-accent-3)] text-white shadow-lg shadow-rose-500/20">
        ...
        <Card className="space-y-5">
          ...
          <form className="space-y-4" onSubmit={handleSubmit} noValidate>
            ...
            <label className="block space-y-2">
              <span className="text-sm font-bold">确认密码</span>
              <span className="flex min-h-12 items-center gap-3 rounded-2xl border border-[var(--ql-card-border)] bg-[var(--ql-soft)] px-3 focus-within:ring-2 focus-within:ring-[var(--ql-accent)]">
                <LockKeyhole className="h-4 w-4 text-[var(--ql-muted)]" />
                <input
          ...
          <p className="text-center text-sm text-[var(--ql-muted)]">
            已经有账号？
            <Link className="font-bold text-[var(--ql-accent)] underline underline-offset-4" to={`/login?redirect=${encodeURIComponent(redirectTo)}`}>
              去登录
            </Link>
          </p>
        </Card>
