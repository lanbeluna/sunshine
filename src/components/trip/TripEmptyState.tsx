...
export function TripEmptyState() {
  return (
    <div className="flex flex-col items-center px-8 pb-12 pt-8 text-center">
      <motion.div
        initial={{ opacity: 0, scale: 0.96 }}
        animate={{ opacity: 1, scale: 1 }}
        className="mb-6 w-full max-w-[200px]"
      >
        <svg viewBox="0 0 200 160" className="w-full text-indigo-400/85" aria-hidden>
          <defs>
      ...
      <p className="max-w-[260px] text-sm leading-relaxed text-wander-secondary">还没有行程哦，去灵感页生成一个吧</p>
      <Link
        to="/decision"
        onClick={() => toast.success('前往灵感', { description: '用问答或盲盒生成你的下一段行程灵感。' })}
        className="mt-6 flex h-12 w-full max-w-xs items-center justify-center rounded-xl bg-gradient-to-r from-indigo-500 to-purple-500 font-semibold text-white shadow-lg shadow-indigo-500/25 transition active:scale-[0.98]"
      >
        去决策
      </Link>
    </div>
