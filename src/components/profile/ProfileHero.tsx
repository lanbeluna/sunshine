...
export function ProfileHero() {
  ...
  const waveFill = light ? '#f4f4f5' : '#0f0f0f';

  return (
    <section className="relative px-4 pb-2">
      <div className="relative overflow-hidden rounded-b-[2rem]">
        <div className="h-40 bg-gradient-to-b from-zinc-800 to-zinc-950 dark:from-zinc-900 dark:to-black" />
      ...
      <div
        className={cn(
          'relative z-[1] -mt-14 mx-auto max-w-sm rounded-2xl border px-4 pb-5 pt-12 shadow-lg',
          light
            ? 'border-zinc-200/80 bg-white text-zinc-900 shadow-zinc-300/40'
            : 'border-white/10 bg-wander-card text-white shadow-black/20'
        )}
      >
        <div className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2">
          <div className="relative">
            <div
              className={cn(
                'h-20 w-20 overflow-hidden rounded-full border-[3px] border-white shadow-xl ring-2 ring-indigo-500/20',
            ...
            <button
              ...
              className="absolute -bottom-0.5 -right-0.5 flex h-7 w-7 items-center justify-center rounded-full border-2 border-wander-card bg-wander-brand text-white shadow-md transition active:scale-95"
              aria-label="编辑头像"
            >
        ...
        <div className="pt-2 text-center">
          ...
          <p className={cn('mt-0.5 text-xs', light ? 'text-zinc-500' : 'text-wander-muted')}>@ql_lightrip</p>
          <p
            className={cn(
              'mx-auto mt-2 max-w-[280px] text-sm leading-relaxed',
              light ? 'text-zinc-600' : 'text-wander-secondary'
            )}
          >
            轻旅随行，灵感即达
          </p>
